namespace Yops.AuditStack.Test
{
    ...
    public class AuditTest
    {
        ...
        {
            // Arrange
            string operation = "saveOperation";
            int page = 1;
            int size = 2;

            // Act
            var auditList = Dog.AuditGetByOperation(operation, page, size);

            // Assert
            Assert.IsNotNull(auditList);
            Assert.AreEqual(auditList[0].Operation, operation);
        }

        [Test]
        [Order(7)]
        public async Task GetAuditByOperationAsync()
        {
            // Arrange
            string operation = "saveOperation";
            int page = 1;
            ...
            // Act
            var auditList = await Dog.AuditGetByOperationAsync(operation, page, size, cancellationToken.Token);
